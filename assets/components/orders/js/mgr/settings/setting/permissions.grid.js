orders.grid.Permissions = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'orders-grid-settings-permissions';
    }
    Ext.applyIf(config, {
    ...
    orders.grid.Permissions.superclass.constructor.call(this, config);
...
Ext.extend(orders.grid.Permissions, MODx.grid.Grid, {
    windows: {},
...
Ext.reg('orders-grid-settings-permissions', orders.grid.Permissions);
