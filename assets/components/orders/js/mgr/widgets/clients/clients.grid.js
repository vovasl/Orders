orders.grid.Clients = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'orders-grid-clients';
    }
    Ext.applyIf(config, {
    ...
    orders.grid.Clients.superclass.constructor.call(this, config);
...
Ext.extend(orders.grid.Clients, MODx.grid.Grid, {
    windows: {},
    ...
    createClient: function (btn, e) {
        var w = MODx.load({
            xtype: 'orders-client-window-create',
            id: Ext.id(),
            listeners: {
                success: {
                    fn: function () {
        ...
        MODx.Ajax.request({
            ...
            listeners: {
                success: {
                    fn: function (r) {
                        var w = MODx.load({
                            xtype: 'orders-client-window-update',
                            id: Ext.id(),
                            record: r,
                            listeners: {
                                success: {
    ...
    getFields: function () {
        return ['id', 'name', 'senders', 'manager', 'manager2', 'goods', 'actions'];
    },

    getColumns: function () {
        return [{
            header: _('orders_clients_name'),
            dataIndex: 'name',
            sortable: true,
            width: 150,
            ...
            header: _('orders_managers2_name'),
            dataIndex: 'manager2',
            width: 150,
        }, {
    ...
    getTopBar: function () {
        return [{
            text: '<i class="icon icon-plus"></i>&nbsp;' + _('orders_client_create'),
...
Ext.reg('orders-grid-clients', orders.grid.Clients);
